...
public class RotEventHandler 
{
	...
	public void onLivingUpdateEvent(LivingUpdateEvent event)
	{
		if (event.entity instanceof EntityPlayer)
		{
				...
				if (is != null)
				{
					...
					if (i instanceof ItemSword || i instanceof ItemTool)
					{
						...
						if (rank == 0)
						{
							...
							//UtilityNBTHelper.setFloat(is, Rot.MODID+"size", 0.45f * (player.worldObj.rand.nextInt(2) == 0 ? 1 : -1));
							//UtilityNBTHelper.setFloat(is, Rot.MODID+"size", 0.65f * (player.worldObj.rand.nextInt(2) == 0 ? 1 : -1));
						}
					}
			...
			if (held != null)
			{
				if (held.getItem() instanceof ItemSword || held.getItem() instanceof ItemTool || held.getItem() instanceof ItemBow)
				{
			...
			if (props.getDexterity() != (dexMod - props.getClassModifers()[4]))props.setDexterity(dexMod);
			...
			if (props.getIntelligence() != (intMod - props.getClassModifers()[2]))props.setIntelligence(intMod);
			if (props.getStrength() != (strMod - props.getClassModifers()[0]))props.setStrength(strMod);
			if (props.getVitality() != (vitMod - props.getClassModifers()[3]))props.setVitality(vitMod);
		}
	}
	...
	public void onDamage(LivingHurtEvent event)
	{
		...
		if(event.source instanceof EntityDamageSource)
		{
			...
			if(source.getEntity() instanceof EntityPlayer)
			{
				...
				{
					if (ExtendPlayer.get(player).getCurrentClassName().equals(ExtendPlayer.classNames[1]))
						event.ammount += MathHelper.clamp_float(ExtendPlayer.get(player).getStrength() / 4f, 0f, 5.75f);
					else
				...
				{
					if (ExtendPlayer.get(player).getCurrentClassName().equals(ExtendPlayer.classNames[5]))
						event.ammount += MathHelper.clamp_float(ExtendPlayer.get(player).getDexterity() / 6f, 0f, 4.5f);
					else
		...
		{
			EntityPlayer player = (EntityPlayer)event.entity;
			if (ExtendPlayer.get(player).getCurrentClassName().equals(ExtendPlayer.classNames[3]))
				event.ammount = MathHelper.clamp_float(event.ammount - ExtendPlayer.get(player).getVitality() / 4.5f, 0.25f, 8f);
			else
				event.ammount = MathHelper.clamp_float(event.ammount - ExtendPlayer.get(player).getVitality() / 7.5f, 0.25f, 20f);
		}
	}
	...
	{
		if (ExtendPlayer.get(e.entityPlayer).getCurrentClassName().equals(ExtendPlayer.classNames[4]))
		{
			e.newSpeed += (MathHelper.clamp_float((ExtendPlayer.get(e.entityPlayer).getStrength() * 2),0f,30f));
		}
		else
		{
			e.newSpeed += (MathHelper.clamp_float((ExtendPlayer.get(e.entityPlayer).getStrength() / 3),-0.2f,9f));
		}
	...
	public void onItemToolTipUpdate(ItemTooltipEvent e)
	{
		int rank = UtilityNBTHelper.getInt(e.itemStack, Rot.MODID+"rankLevel"), 
				...
				inte = UtilityNBTHelper.getInt(e.itemStack, Rot.MODID+"intModifier"),
		...
		if (e.itemStack.getItem() instanceof ItemTool || e.itemStack.getItem() instanceof ItemSword || e.itemStack.getItem() instanceof ItemArmor || e.itemStack.getItem() instanceof ItemBow)
		{
			...
			if (inte != 0)e.toolTip.add((inte > 0 ? EnumChatFormatting.GREEN : EnumChatFormatting.RED)+"Intelligence Modifier: "+inte);
	...
	public void onEntityConstructing(EntityConstructing event)
	{
		...
		if (event.entity instanceof EntityPlayer && ExtendPlayer.get((EntityPlayer) event.entity) == null)
