...
public class GuiBaseNode extends GuiContainer
{
	...
	private int startLeft = this.cw * 2, startTop = this.ch * 2;
	...
	protected void actionPerformed(GuiButton button)
	{
		...
		if (button.id < this.indexCounter)
		{
			...
			case 0: // Start Building
				Rot.net.sendToServer(new BaseNodeRequestPacket(2, this.teX, this.teY, this.teZ, 0,
						0, 0, 0));
				break;
			case 1: // Send List
				if (!this.locations.isEmpty())
				{
					...
					for (int l = 0; l < this.locations.size(); l++)
					{
						...
						Rot.net.sendToServer(new BaseNodeRequestPacket(0, this.teX, this.teY,
								this.teZ, ublt.x, ublt.y, ublt.z, Block.getIdFromBlock(ublt.block)));
					}
				}
				this.locations.clear();
				break;
			case 2: // -X left/west
				Rot.net.sendToServer(new BaseNodeRequestPacket(1, this.teX, this.teY, this.teZ, 0,
						0, 0, 0));
				this.locations.clear();
				break;
			case 3: // +X right/east
			...
			case 15:
				...
				break;
			case 16:
				Rot.net.sendToServer(new BaseNodeRequestPacket(3, this.teX, this.teY, this.teZ, 0,
						0, 0, 0));
				break;
			}
		}
		...
		else
		{
			...
			if (this.selectionMode == 1)// Range Mode
			{
				...
				{
					int xh = (this.AB[0].x > this.AB[1].x ? (int) this.AB[0].x : (int) this.AB[1].x);
					int xl = (this.AB[0].x < this.AB[1].x ? (int) this.AB[0].x : (int) this.AB[1].x);

					int yh = (this.AB[0].y > this.AB[1].y ? (int) this.AB[0].y : (int) this.AB[1].y);
					int yl = (this.AB[0].y < this.AB[1].y ? (int) this.AB[0].y : (int) this.AB[1].y);

					int zh = (this.AB[0].z > this.AB[1].z ? (int) this.AB[0].z : (int) this.AB[1].z);
					int zl = (this.AB[0].z < this.AB[1].z ? (int) this.AB[0].z : (int) this.AB[1].z);
					for (int xs = xh; xs >= xl; xs--)
					{
						for (int zs = zh; zs >= zl; zs--)
						{
							...
							{
								addLocation(xs + this.teX, ys + this.teY, zs + this.teZ);
							}
						}
					}
					this.AB = new Vector3f[2];
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j)
	{
		...
		int gx1 = this.startLeft - 3, gx2 = (this.startLeft + (this.gridSize * this.cw)) + (this.gridSize * this.cw) + this.cw + 3, gy1 = this.startTop - 3, gy2 = gy1 + (this.gridSize * this.ch) + (this.gridSize * this.ch) + this.ch + 6;
		int gw = ((this.gridSize * 2) * this.cw) + (this.cw + 6), gh = ((this.gridSize * 2) * this.ch) + (this.ch + 6);
		...
		drawTexturedModalRect(gx1, gy1 + (gh / 2), 0, 226 - (gh / 2), gw / 2, gh / 2);// lower
																						// left
		drawTexturedModalRect(gx1 + (gw / 2), gy1 + (gh / 2), 227 - (gw / 2), 226 - (gh / 2),
		...
		drawTexturedModalRect(gx2, gy1 + (gh / 2), 0, 226 - (gh / 2), gw / 2, gh / 2);// lower
																						// left
		drawTexturedModalRect(gx2 + (gw / 2), gy1 + (gh / 2), 227 - (gw / 2), 226 - (gh / 2),
				gw / 2, gh / 2);// lower right
		...
		this.buttonList.add(new GuiButton(3, gx1, gy2 + this.ch, 60, this.ch, "< Block"));// prev
																							// block
		this.buttonList.add(new GuiButton(4, gx1 + 60, gy2 + this.ch, 60, this.ch, "Block >"));// next
																								// block
		this.buttonList.add(new GuiButton(5, gx1 + (75 * 3), gy2, 60, this.ch, "Grid +"));// prev
																							// block
		this.buttonList.add(new GuiButton(6, gx1 + (75 * 3) + 60, gy2, 60, this.ch, "Grid -"));// next
																								// block
		this.buttonList.add(new GuiButton(7, gx1 + (60 * 2), gy2 + this.ch, 90, this.ch,
				this.selectionTitle[this.selectionMode]));// selection mode
		this.buttonList.add(new GuiButton(16, gx1 + (60 * 2) + 90, gy2 + this.ch, 75, this.ch,
				"Get List"));// selection mode

		this.buttonList.add(new GuiButton(8, (gx1 + gx2 + gw) / 2, gy1 - this.ch, this.cw, this.ch,
				"<"));// X left
		this.buttonList.add(new GuiButton(9, ((gx1 + gx2 + gw) / 2) + (this.cw * 3), gy1 - this.ch,
				this.cw, this.ch, ">"));// X right
		this.buttonList.add(new GuiButton(10, ((gx1 + gx2 + gw) / 2) + this.cw, gy1 - this.ch,
				this.cw, this.ch, "^"));// Z 'forward'
		this.buttonList.add(new GuiButton(11, ((gx1 + gx2 + gw) / 2) + (this.cw * 2),
				gy1 - this.ch, this.cw, this.ch, "v"));// Z 'back'
		this.buttonList.add(new GuiButton(12, gx1 - this.cw,
				(this.startTop + (this.gridSize * this.ch)) - (this.ch / 2), this.cw, this.ch,
				"Y1+"));// Y1 up
		 ...
		 * ch, 0xFFFFFF); this.drawString(fontRendererObj, "OffSet: "+yOffset1,
		 * (startLeft + ((gridSize * 2) * cw)) + cw * 6, (startTop + (gridSize *
		 * ch) + 4), 0xFFFFFF); this.drawString(fontRendererObj,
		 * "OffSet: "+yOffset2, (startLeft + ((gridSize * 2) * cw)) + cw * 11,
		 * (startTop + (gridSize * ch) + 4), 0xFFFFFF);
		 * this.drawString(fontRendererObj, "OffSet: "+zOffset, (startLeft +
		 * ((gridSize * 2) * cw)) + cw * 6, (startTop + (gridSize * ch) + 4) +
		 * ch, 0xFFFFFF); this.drawString(fontRendererObj, (AB == null ?
		 * "single Mode":(AB[0] == null ? "Point A not selected" :
		 * AB[0])).toString(), (startLeft + ((gridSize * 2) * cw)) + cw * 8,
		 * (startTop + ((gridSize * 2) * ch)) - ch * 3, blockColor); // What
		 * block is selected
		 */

		this.drawString(this.fontRendererObj, UtilityFunctions.blockTypeObjects[this.currentBlock]
				.getLocalizedName(), gx1, gy2 + (this.ch * 2), this.blockColor); // What
																					// block
																					// is
																					// selected
	...
	private void addLocation(int x, int y, int z)
	{
		...
		else
		{
			...
			for (int l = 0; l < this.locations.size(); l++)
			{
				UtilityBlockLocationType ublt = (UtilityBlockLocationType) this.locations.get(l);
	...
	{
		if ((this.coordButtons1 == null) || (this.coordButtons1.length != (((this.gridSize * 2) + 1) * ((this.gridSize * 2) + 1))))
		{
			this.coordButtons1 = new GuiBaseNodeButton[((this.gridSize * 2) + 1) * ((this.gridSize * 2) + 1)];
		}

		if ((this.coordButtons2 == null) || (this.coordButtons2.length != (((this.gridSize * 2) + 1) * ((this.gridSize * 2) + 1))))
		{
			this.coordButtons2 = new GuiBaseNodeButton[((this.gridSize * 2) + 1) * ((this.gridSize * 2) + 1)];
		}
		...
		for (int x = this.gridSize; x >= -this.gridSize; x--)
		{
			for (int z = this.gridSize; z >= -this.gridSize; z--)
			{
				...
				String s1 = "x";
				String s2 = "x";
				Block worldBlock1 = this.te.getWorld().getBlockState(
						new BlockPos(x + this.teX + this.xOffset, this.yOffset1 + this.teY,
								z + this.teZ + this.zOffset)).getBlock();
				Block worldBlock2 = this.te.getWorld().getBlockState(
						new BlockPos(x + this.teX + this.xOffset, this.yOffset2 + this.teY,
								z + this.teZ + this.zOffset)).getBlock();
				if (this.selectionMode == 1)
				{
					if ((this.AB != null) && (this.AB[0] != null) && ((x + this.xOffset) == (int) this.AB[0].x) && ((z + this.zOffset) == (int) this.AB[0].z))
					{
						s1 = "X";
						s2 = "X";
					}
				...
				if (!this.locations.isEmpty())
				{
					...
					for (int ubltl = 0; ubltl < this.locations.size(); ubltl++)
					{
						...
						{
							if ((ubltS.x == (x + this.xOffset + this.teX)) && (ubltS.z == (z + this.zOffset + this.teZ)))
							{
								s1 = "*";
				...
				this.coordButtons1[buttonArrayIndex] = new GuiBaseNodeButton(this.indexCounter++,
						(this.startLeft + (this.gridSize * this.cw)) + ((this.cw * x)),
						(this.startTop + (this.gridSize * this.ch)) + ((this.ch * z)), this.cw,
						this.ch, s1);

				this.coordButtons2[buttonArrayIndex] = new GuiBaseNodeButton(
						this.indexCounter++,
						((this.startLeft + (this.gridSize * this.cw)) + ((this.gridSize * this.cw) * 2) + this.cw + 6) + ((this.cw * x)),
						(this.startTop + (this.gridSize * this.ch)) + ((this.ch * z)), this.cw,
						this.ch, s2);
				if (((x + this.xOffset) == 0) && (this.yOffset1 == 0) && ((z + this.zOffset) == 0))
				{
					this.coordButtons1[buttonArrayIndex].packedFGColour = 0x0000FF;
				}
				else
				{
					this.coordButtons1[buttonArrayIndex].packedFGColour = c1 == this.defaultColor ? (worldBlock1
							.equals(Blocks.air) ? 0x00CCFF : worldBlock1.getRenderColor(worldBlock1
							.getDefaultState())) : c1;
				}
				if (((x + this.xOffset) == 0) && (this.yOffset2 == 0) && ((z + this.zOffset) == 0))
				{
					this.coordButtons2[buttonArrayIndex].packedFGColour = 0x0000FF;
				}
				else
				{
					this.coordButtons2[buttonArrayIndex].packedFGColour = c2 == this.defaultColor ? (worldBlock2
							.equals(Blocks.air) ? 0x00CCFF : worldBlock2.getRenderColor(worldBlock2
							.getDefaultState())) : c2;
				}
				TextureAtlasSprite tex1 = null, tex2 = null;
				float a1 = 1.0f, a2 = 1.0f;
				for (float depth = 0; depth < 10; depth++)
				{
					Block tempBlock = this.te.getWorld().getBlockState(
							new BlockPos(this.teX + this.xOffset + x, this.teY - depth + this.yOffset1,
									this.teZ + this.zOffset + z)).getBlock();
					boolean isUnique = tempBlock instanceof BlockLiquid || tempBlock instanceof BlockFire;
					if (tempBlock != Blocks.air && !isUnique)
					{
						a1 -= (depth / 10);
						tex1 = Minecraft.getMinecraft().getRenderItem().getItemModelMesher().getItemModel(
								new ItemStack(tempBlock)).getTexture();
						break;
					}
				}
				...
				{
					Block tempBlock = this.te.getWorld().getBlockState(
							new BlockPos(this.teX + this.xOffset + x, this.teY - depth + this.yOffset2,
									this.teZ + this.zOffset + z)).getBlock();
					boolean isUnique = tempBlock instanceof BlockLiquid || tempBlock instanceof BlockFire;
					if (tempBlock != Blocks.air && !isUnique)
					{
						a2 -= (depth / 10);
						tex2 = Minecraft.getMinecraft().getRenderItem().getItemModelMesher().getItemModel(
								new ItemStack(tempBlock)).getTexture();
						break;
					}
				}
				...
				/*
				 * while (t1 == null) { t1 = this.te .getWorldObj() .getBlock(x
				 * + this.te.xCoord + this.xOffset, (this.yOffset1 +
				 * this.te.yCoord) - depth1, z + this.te.zCoord +
				 * this.zOffset).getIcon(1, 0); if (t1 == null) { if
				 * (!this.locations.isEmpty()) { // Look through every Item of
				 * the list for (int ubltl = 0; ubltl < this.locations.size();
				 * ubltl++) { ublt = (UtilityBlockLocationType) this.locations
				 * .get(ubltl); if (ublt.y == ((this.yOffset1 + this.te.yCoord)
				 * - depth1)) { if ((ublt.x == (x + this.xOffset +
				 * this.te.xCoord)) && (ublt.z == (z + this.zOffset +
				 * this.te.zCoord))) { t1 = ublt.block.getIcon(1, 0); break; } }
				 * } } MathHelper.clamp_float(b1 -= 0.2f, 0, 1f); depth1++; } }
				 * while (t2 == null) { t2 = this.te .getWorldObj() .getBlock(x
				 * + this.te.xCoord + this.xOffset, (this.yOffset2 +
				 * this.te.yCoord) - depth2, z + this.te.zCoord +
				 * this.zOffset).getIcon(1, 0); if (t2 == null) { if
				 * (!this.locations.isEmpty()) { // Look through every Item of
				 * the list for (int ubltl = 0; ubltl < this.locations.size();
				 * ubltl++) { ublt = (UtilityBlockLocationType) this.locations
				 * .get(ubltl); if (ublt.y == ((this.yOffset2 + this.te.yCoord)
				 * - depth2)) { if ((ublt.x == (x + this.xOffset +
				 * this.te.xCoord)) && (ublt.z == (z + this.zOffset +
				 * this.te.zCoord))) { t2 = ublt.block.getIcon(1, 0); break; } }
				 * } } MathHelper.clamp_float(b2 -= 0.2f, 0, 1f); depth2++; } }
				 */
				this.coordButtons1[buttonArrayIndex].tex = tex1;
				...
				this.coordButtons1[buttonArrayIndex].alpha = a1;
				this.coordButtons1[buttonArrayIndex].block = worldBlock1;
				this.coordButtons1[buttonArrayIndex].canSeeSky = te.getWorld().canBlockSeeSky(new BlockPos(x + this.xOffset + teX, this.yOffset1 + teY, z + this.zOffset + teZ));

				this.coordButtons2[buttonArrayIndex].tex = tex2;
				...
				this.coordButtons2[buttonArrayIndex].alpha = a2;
				this.coordButtons2[buttonArrayIndex].block = worldBlock2;
				this.coordButtons2[buttonArrayIndex].canSeeSky = te.getWorld().canBlockSeeSky(new BlockPos(x + this.xOffset + teX, this.yOffset2 + teY, z + this.zOffset + teZ));
				buttonArrayIndex++;
			}
		}
