...
public class GuiExtendedPlayerStats extends Gui
{
	...
	public void onRenderExperienceBar(RenderGameOverlayEvent event)
	{
		...
		int currentbarwidth = MathHelper
				.clamp_int((int) (((float) props.getCurrentMana() / props
						.getMaxMana()) * this.barW), 0, this.barW);

		drawTexturedModalRect(xPos, yPos, 0, this.barH, currentbarwidth,
				this.barH);
		String s = (int) props.getCurrentMana() + "/"
				+ (int) props.getMaxMana();
		...
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		drawTexturedModalRect(xPos + this.barW + 1, yPos, 0, 0, this.barW,
				this.barH);

		currentbarwidth = MathHelper
				.clamp_int((int) (((float) props.getCurrentStam() / props
						.getMaxStam()) * this.barW), 0, this.barW);

		drawTexturedModalRect(xPos + this.barW + 1, yPos, 0, this.barH,
				currentbarwidth, this.barH);
		...
		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		drawTexturedModalRect(xPos + (this.barW * 2) + 2, yPos, 0, 0,
				this.barW, this.barH);

		float adjustedHP = (this.mc.thePlayer.getMaxHealth() * 5)
				+ props.pickedClass.baseHp
				+ (props.pickedClass.hpPerVit * props.getVitality());
		float currentHPPercent = this.mc.thePlayer.getHealth()
				/ this.mc.thePlayer.getMaxHealth();

		currentbarwidth = MathHelper.clamp_int((int) (currentHPPercent)
				* this.barW, 0, this.barW);

		drawTexturedModalRect(xPos + (this.barW * 2) + 2, yPos, 0, this.barH,
				currentbarwidth, this.barH);
