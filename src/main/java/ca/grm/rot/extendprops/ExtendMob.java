...
public class ExtendMob implements IExtendedEntityProperties
{
	...
	public String bossPrefix4; // An array isn't used here because it's simpler
								// to just use these. The max # of prefixes
								// isn't dynamic and its not massive.
	...
	private RotMobAffix rollPrefix()
	{
		...
		return RotMobAffixManager.getAffix(this.monsterLevel, RotMobAffixManager
				.getRandomPrefixArray(), random);
	}
	...
	private RotMobAffix rollSuffix()
	{
		Random random = new Random();
		return RotMobAffixManager.getAffix(this.monsterLevel, RotMobAffixManager
				.getRandomSuffixArray(), random);
	}

	public void rollAffixes()
	{
		if (this.isBoss()) // If its a boss, give it a prefix, up to 3 more
							// prefixes, and maybe suffix.
		{
		...
		// If its not a boss, never give a suffix and maybe a prefix.
		{
		...
		applyAffixStats(); // This just makes the affixes do stuff. Mostly
							// prefixes.
	}
	...
	public void rollStats(int depth)
	{
		...
		monsterLevel += (int) (((strength + dexterity + agility + vitality + minDmg + maxDmg + defBonus) / 7)); // /4
																												// gets
																												// the
																												// average,
																												// the
