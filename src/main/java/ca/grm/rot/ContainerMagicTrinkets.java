...
public class ContainerMagicTrinkets extends Container {
	 ...
	 */
	private static final int	ARMOR_START	= InventoryMagicTrinkets.INV_SIZE,
			ARMOR_END = ARMOR_START + 3, INV_START = ARMOR_END + 1,
			INV_END = INV_START + 26, HOTBAR_START = INV_END + 1,
			HOTBAR_END = HOTBAR_START + 8;

	public ContainerMagicTrinkets(EntityPlayer player, InventoryPlayer inventoryPlayer,
			InventoryMagicTrinkets inventoryTrinket) {
		int i;
		...
		// you need
		for (i = 0; i < 4; ++i) {
			this.addSlotToContainer(new Slot(inventoryPlayer, inventoryPlayer
					.getSizeInventory() - 1 - i, 8, 8 + (i * 18)));
		}
